...
import { atomSetAudioContextIfNeededAsync } from "./_atomSetAudioContextIfNeeded";
import { play } from "./_play";
import { atomDevicesAsync } from "./_atomDevices";
...
export const atomPlay = {
  ...
  getDevicesAsync: atom(async (get) => {
    const devices = await get(atomDevicesAsync); // デバイスの取得をatom化することで、atomDeviceId変更時に再取得するのを防ぐ
  ...
  playAsync: atom(null, async (get, set, volume: number = 1) => {
    await set(atomSetAudioContextIfNeededAsync);
